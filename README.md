...
- **Admin operations assistant** — a staff-only, read-only chat under **Admin > Assistant** that looks up permitted jobs, AI quality-review flags, tank-gauge stock readings, and reference documents, citing every claim with a `[Ref n]` link back to the underlying record. It cannot approve, assign, notify, or otherwise change anything directly. Runs in keyword-routed lookup mode by default; once an administrator validates a provider for agent tool calls, it runs as a bounded, step/time/token-limited agent that chooses its own tool calls instead — including, when asked, filing a job-approval proposal for a human to review under **Admin > Reviewed Actions** rather than approving it itself.
...
- Four of the agent's tools are the read-only lookups from the Admin operations assistant above (jobs, AI review flags, stock readings, reference documents) — nothing here can write to any table. A fifth tool, `propose_job_approval`, is the one step-5 "reviewed action" wired in so far: it never approves anything itself, only files a durable proposal a human must separately review; see the Reviewed actions section below.
...
### Reviewed actions — implemented (one action type)

Roadmap step 5 ("Introduce reviewed actions"). See [Reviewed actions](docs/reviewed-actions.md) for the proposal lifecycle, the permission/staleness rechecks, and the idempotency guarantee.

- `src/lib/reviewed-actions.ts` adds a durable, human-reviewable proposal pipeline: propose → (recheck permission + target state fresh) → approve/reject → execute → record the result. Every transition happens on one `proposed_actions` row (migration: `npm run db:migrate:reviewed-actions`), so a single query shows a proposal's complete history — who proposed it, who decided it and when, what the target looked like at both proposal and approval time, and the execution outcome.
- **Reuses business services and job-workflow transition rules exactly, rather than reimplementing them.** The original `approveJob` Server Action's logic was extracted verbatim into `src/lib/job-actions.ts`'s `approveJobCore()` — the direct staff-facing "Approve" button on a job and a reviewed-action's execution now call the identical function. Both proposing and approving independently re-run the same `canTransition()` job-workflow rule already used everywhere else in the codebase; a proposal is never even created for a job that isn't awaiting approval, and approving re-checks that rule again in case something changed in between.
- **Permission is rechecked at approval time, not inherited from the proposal.** Each action type declares its own required staff roles; approving or rejecting looks that up fresh for the specific reviewer, not "any staff member who can see the queue."
- **State is rechecked and recorded at approval time.** The target's current state is re-verified independently of the snapshot taken when the proposal was made (`proposedState`) — if it no longer matches (e.g. someone already decided the job another way), the proposal is marked `stale` and never executes. On a successful approval, a fresh snapshot (`approvedState`) is recorded alongside the decision, distinct from the original proposal-time snapshot.
- **Duplicate execution on retries is prevented by atomic, status-guarded claims**, not by trusting the caller not to retry: every transition is a conditional database update (`WHERE status = '<expected>'`) that only one caller can win. A retried approval call on an already-decided proposal safely no-ops rather than running the underlying action again.
- **Still under explicit staff control**, per the roadmap: the agent's new `propose_job_approval` tool (`src/lib/ai/agent-tools.ts`) can only create a proposal — actually approving one requires a human on **Admin > Reviewed Actions**, which is where the inspection approval itself happens (issuing the Certificate of Quantity and invoice, and notifying the client — three of the roadmap's four named categories, all through the one reused `approveJobCore()` flow). Payment verification, report issuance as its own action, and other outbound messages are not yet wired to this pipeline; `HANDLERS` in `reviewed-actions.ts` is a small registry designed so each would be one more entry reusing its own existing business service, the same way `job_approval` reuses `approveJobCore()`.

Validation completed for this change: production build, TypeScript checking, targeted ESLint, and `node scripts/test-reviewed-actions.cjs`. Tests cover `job-actions.ts` directly (ref lookup, the real `job-workflow.ts` transition rule reused rather than reimplemented, and the full approval side effects — status, timeline entries, Certificate of Quantity/invoice generation, and cache revalidation) and `reviewed-actions.ts` in isolation from any one action type's business logic (a stubbed `job-actions.ts`): proposing refuses to create a proposal for a non-transitionable job; approving rechecks the transition rule a second time at approval time and records an approval-time state snapshot; a reviewer lacking the action type's required role is refused before anything executes; a target that changed between proposal and approval is marked stale rather than executed; a retried approval call on an already-decided proposal never re-executes the underlying action; an execution failure is durably recorded rather than lost; and rejecting requires a note, rechecks permission, and cannot be redone once settled. Not yet live-verified in the running application — do that (propose a job approval via the assistant in agent mode, then approve and reject test proposals from Admin > Reviewed Actions) before relying on it operationally, and only after validating a provider for agent tool calls as described in the Bounded agent runner section above.
...
5. ~~**Introduce reviewed actions.**~~ Started — the durable proposal/approval/execution pipeline is implemented and tested (permission and state rechecked at approval time, idempotent execution, durable records), wired to one action type, job approval; see the Reviewed actions section above and [docs/reviewed-actions.md](docs/reviewed-actions.md). Payment verification, report issuance as its own action, and external messages still need their own registry entries. Not yet live-verified in the running application.
