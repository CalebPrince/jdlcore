...
import { academyLearners, analyticsUsers, clients, passwordResetTokens } from "@/db/schema";
import { sendNotification } from "@/lib/email";
import { hashPassword } from "@/lib/portal-auth";

export type RecoveryState = { ok: boolean; message: string; loginHref?: string };
type AccountType = "academy" | "analytics" | "portal";
...
export async function requestPasswordReset(_previous: RecoveryState, formData: FormData): Promise<RecoveryState> {
  const parsed = z.object({ accountType: z.enum(["academy", "analytics", "portal"]), email: z.string().trim().email().max(200) }).safeParse(Object.fromEntries(formData));
...
export async function resetPassword(_previous: RecoveryState, formData: FormData): Promise<RecoveryState> {
  ...
  const loginHref = accountType === "academy" ? "/academy/login" : accountType === "analytics" ? "/analytics/login" : "/portal/login";
