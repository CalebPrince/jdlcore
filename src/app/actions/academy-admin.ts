...
export async function createAcademyModule(formData: FormData) {
  ...
  await requireDb().insert(academyModules).values({ courseId, title, description: description || null, position: existing.length });
